...
export function registerGetUserRecentPagesTool(server: AnyFastMCP): void {
  server.addTool({
    ...
    execute: async (params) => {
      try {
        ...
        const result = await apiv3.getRecentByIdForUsers(getUserRecentPagesParams.id, { appName: resolvedAppName });
